...
class ListBaton : public Nan::AsyncResource {
public:
  ...
  ListBaton(v8::Local<v8::Function> callback_) : AsyncResource("node-serialport:ListBaton") {
...
void DoList(uv_work_t* req) {

  auto details = static_cast<ListBaton*>(req->data);  
  details->list();

}

void AfterList(uv_work_t* req, int status) {
  Nan::HandleScope scope;

  auto data = static_cast<ListBaton*>(req->data);
...
NAN_METHOD(List) {
    ...
    uv_queue_work(Nan::GetCurrentEventLoop(), &baton->request, DoList, reinterpret_cast<uv_after_work_cb>(AfterList));
}
